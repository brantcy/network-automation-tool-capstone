<#
#FOR TESTING
  PasswordChangeNotification.ps1 -smtpServer 0.0.0.0 -expireInDays 7 -from "IT Support <it@somecompany.com>" -Logging -LogPath "D:\scripts\logFiles" -testing -testRecipient some.user@somecompany.com
  
  This example will use mail.domain.com as an smtp server, notify users whose password expires in less than 7 days, send mail from it@somecompany.com
  Logging is enabled, log path is c:\scripts\
  Testing is enabled, and test recipient is some.user@somecompany.com

#FOR LIVE
  PasswordChangeNotification.ps1 -smtpServer 0.0.0.0 -expireInDays 7 -from "IT Support <it@somecompany.com>" -reportTo alerts@somecompany.com -interval 1,2,5,7
  
  This example will use mail.domain.com as an smtp server, notify users whose password expires in less than 7 days, send mail from it@somecompany.com
  Report is enabled, reports sent to it@somecompany.com
  Interval is used, and emails will be sent to people whose password expires in less than 7 days if the script is run, with 7, 5, 2 or 1 days remaining untill password expires.

  This is set into the Task Schedule at 7am every day
...
param(
    ...
    [ValidateNotNull()]
    [int]$expireInDays,
    # From Address, eg "IT Support <it@somecompany.com>"
...
foreach ($user in $notifyUsers)
{
    ...
    To avoid disruption to your access please change your password via a Desktop or Laptop press CTRL+ALT+Delete and choose ""Change a Password"" <br>
    <p> <b>If you are out of the office please make sure you are connected to the VPN before proceeding.</b><br>
    <p> Don't forget to update the password on your mobile devices as well!
    <p>Thanks, <br> 
    </P>
    IT<br><br>
    <b>If you have any problems please email <a href=""mailto:it@somecompany.com?Subject=Password Expiry Assistance"">it@somecompany.com</a> <br> or call 01283 560266 <br><br><br>
    </font></b>"
...
Write-Output "Script Runtime: $runtime 
                please check the log for any errors
                any fatal errors contact some.user@somecompany.com"
# End